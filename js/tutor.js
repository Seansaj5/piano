/* Clef: the tutor in the corner.
   ...
   Claude: reads a photo or PDF of sheet music and holds a real conversation. That path talks to api.anthropic.com
...
If the image is not sheet music, set kind to "not_music" and explain in summary.`;
  ...
  async function callClaude(opts) {
    ...
    const body = { model: m, max_tokens: opts.scan ? 16000 : 8000, stream: true, system: [{ type: "text", text: SYSTEM, cache_control: { type: "ephemeral" } }], messages: opts.messages };
    // Haiku 4.5 takes neither adaptive thinking nor effort. Reading a page deserves more thought than a chat reply.
    if (m !== "claude-haiku-4-5") { body.thinking = { type: "adaptive" }; body.output_config = { effort: opts.scan ? (cfg.careful ? "high" : "medium") : "low" }; }
    if (opts.scan && opts.structured) body.output_config = Object.assign(body.output_config || {}, { format: { type: "json_schema", schema: SCAN_SCHEMA } });
  ...
  async function callWithRetries(opts, produced) {
    try { return await callClaude(Object.assign({ useFallbacks: true, structured: true }, opts)); }
      ...
      if (e && e.status === 400 && !produced()) {
        ...
        if (opts.scan && /output_config|format|schema/i.test(e.message || "")) return await callClaude(Object.assign({ useFallbacks: false, structured: false }, opts));
      }
      throw e;
    }
  }
  ...
  function greet() {
    ...
    const h = addMsg("bot", fmt(aiOn()
      ? "I'm **Clef**. Send me a photo of the music on your stand and I'll break it down: key, chords, the hard bars, and a plan for learning it. Or ask me anything about chords, keys and lead sheets."
      : "I'm **Clef**. I can answer chord, key and scale questions right now, offline. To have me **read a photo of your sheet music** and coach you through a song, add a Claude API key in the settings."));
    const st = document.createElement("div"); st.className = "starters";
    st.innerHTML = [["Scan my sheet music", "scan"], ["What notes are in F#m7b5?", "q"], ["What key has 3 flats?", "q"], ["How do I play from a lead sheet?", "q"], ["Explain ii–V–I", "q"]].map(s => `<button type="button" data-start="${s[1]}">${esc(s[0])}</button>`).join("");
    h.el.appendChild(st);
  ...
  const Tutor = W.Tutor = {
    start() {
      ...
      $("#clefNew").addEventListener("click", () => { if (busy && ctl) ctl.abort(); history = []; lastContext = ""; lastScan = null; shot = null; paintShot(); greet(); });
      ...
      $("#clefLog").addEventListener("click", e => {
        ...
        const s = e.target.closest("[data-start]"); if (s) { if (s.getAttribute("data-start") === "scan") $("#clefFile").click(); else send(s.textContent); }
      });
